...
const Amrap: React.FC = () => {
  ...
  const computeHeat = (offset: number): HeatState => {
    ...
    if (rel < 0) {
      ...
      return { display: formatMMSS(untilGo), active: false, beep: null, finished: false };
    }
    ...
    if (t >= 0 && t < totalSeconds) {
      ...
      else if (t === 0) beep = 'final';
      return { display: formatMMSS(remaining), active: true, beep, finished: false };
    }
    return { display: '00:00', active: false, beep: t === totalSeconds ? 'final' : null, finished: true };
  };
    ...
    <TimerScreen
      ...
      heat1={
        <TimerDisplay
          time={heat1.display}
          fontSize={fontSize}
          label={settings.heatsEnable ? 'HEAT 1' : 'AMRAP'}
        />
      }
      heat2={
        heat2 ? <TimerDisplay time={heat2.display} fontSize={fontSize} label="HEAT 2" /> : null
      }
      belowTimer={
        ended ? (
          <>
